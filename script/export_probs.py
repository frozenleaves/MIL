...
# ======================== 全局配置 ========================
TEST_ROOT = Path("/media/codingma/LLM/lcx/Medical_Info_Classification/datasets/test")
OUT_CSV = Path("/media/codingma/LLM/lcx/Medical_Info_Classification/MIL/script/calibration_long-tests.csv")
THRESHOLD = 0.5
...
# Grad-CAM 可视化开关
SAVE_HEATMAP = True
HEATMAP_DIR = Path("/media/codingma/LLM/lcx/Medical_Info_Classification/MIL/script/heatmap")
...
def main() -> None:
    if not TEST_ROOT.exists():
        raise SystemExit(f"test-root 不存在：{TEST_ROOT}")

    samples = scan_test_root(TEST_ROOT, use_txt=USE_TXT, use_img=USE_IMG, use_svs=USE_SVS)
    if not samples:
        raise SystemExit("未扫描到样本，请检查 test-root 目录结构")

    checkpoint_path = os.path.join("/media/codingma/LLM/lcx/Medical_Info_Classification/checkpoints-70_30_multi_label_txt_img_svs", "best_val.pth")
